...
package com.ljq.privatefiledemo.mutil;
...
import com.ljq.privatefiledemo.privatefile.HideFileListener;
import com.ljq.privatefiledemo.privatefile.HideFileManager;
import com.ljq.privatefiledemo.privatefile.PrivateFileItemBean;
import com.ljq.privatefiledemo.util.AppUtil;
import com.ljq.privatefiledemo.widget.HorizontalProgressBarWithNumber;
import com.ljq.privatefiledemo.widget.SubStateLinearLayout;
...
public class DownloadAdapter extends AbsRVAdapter<AbsEntity, DownloadAdapter.SimpleHolder> {
  ...
  public void refreshData(List<AbsEntity> data){
    mData = data;
  ...
  private void hideFile(AbsEntity entity) {
    if(entity instanceof DownloadEntity){
      DownloadEntity downloadEntity  = (DownloadEntity) entity;
      PrivateFileItemBean bean = HideFileManager.getInstance(mContext).makePrivateFileItemBean(downloadEntity);
      HideFileListener listener = new HideFileListener() {
        @Override
        public void OnSuccess(List<PrivateFileItemBean> beans) {
          mOnChangeListener.hideSuc();
        }

        @Override
        public void onFailed(Exception e, String errorMsg) {
          mOnChangeListener.hideFail();
        }
      };
      HideFileManager.getInstance(mContext).hideSingleFile(bean,listener);
    }
  }
