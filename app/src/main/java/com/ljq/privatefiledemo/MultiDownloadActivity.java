...
import com.ljq.privatefiledemo.base.BaseActivity;
import com.ljq.privatefiledemo.databinding.ActivityMultiDownloadBinding;
import com.ljq.privatefiledemo.mutil.DownloadAdapter;
import com.ljq.privatefiledemo.privatefile.PrivateFileActivity;
...
public class MultiDownloadActivity extends BaseActivity<ActivityMultiDownloadBinding> {
  private final static String TAG = MultiDownloadActivity.class.getSimpleName();
  RecyclerView mList;
  ...
  @Override protected void init(Bundle savedInstanceState) {
    ...
    mAdapter.setOnChangeListener(new DownloadAdapter.onChangeListener() {
      @Override
      public void hideSuc() {
        getDownloadTaskData();
      ...
      public void hideFail() {
        Toast.makeText(MultiDownloadActivity.this,"隐藏文件失败".toString(),Toast.LENGTH_LONG).show();
      }
    });
    ...
    mButton.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        startActivity(new Intent(MultiDownloadActivity.this, PrivateFileActivity.class));
      }
    });
  }
  ...
  private void getDownloadTaskData(){
    mData.clear();
    List<Long> supportBeans = HideFileSupportFileSQLManager.queryHideFileSupportIdList(this);
    List<AbsEntity> temps = Aria.download(this).getTotalTaskList();
    if (temps != null && !temps.isEmpty()) {
      for (AbsEntity temp : temps) {
        ALog.d(TAG, "state = " + temp.getState());
        if(supportBeans.contains(temp.getId())){//如果被隐藏了 则忽略数据
          continue;
        }
        mData.add(temp);
      }
